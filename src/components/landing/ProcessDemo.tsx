...
export default function ProcessDemo() {
  ...
  return (
    ...
    >
      ...
      {/* ── Demo screen (light) ────────────────────── */}
      <div className="relative bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-xl shadow-black/5">
        {/* Title bar */}
        <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-100 bg-gray-50">
          ...
          <div className="flex-1 flex justify-center">
            <div className="text-[10px] text-gray-400 bg-gray-100 px-3 py-0.5 rounded-md">
              quietly.app
            </div>
          </div>
        </div>
        ...
        {/* Content area */}
        <div className="relative h-[380px] sm:h-[420px] overflow-hidden bg-white">
          {/* ── Stage: Upload ──────────────────────── */}
          <div className={`absolute inset-0 p-6 sm:p-8 transition-all duration-500 ${
            ...
            <div className="flex flex-col items-center justify-center h-full gap-6">
              <div className={`w-full max-w-sm border-2 border-dashed rounded-2xl p-8 text-center transition-all duration-700 ${
                uploadPhase === 0 ? "border-indigo-300 bg-indigo-50/50" :
                uploadPhase === 1 ? "border-indigo-400 bg-indigo-50 scale-[0.98]" :
                "border-emerald-400 bg-emerald-50"
              }`}>
                {uploadPhase < 2 ? (
                  <>
                    <div className={`mx-auto w-12 h-12 rounded-xl bg-indigo-100 flex items-center justify-center mb-4 transition-all duration-500 ${uploadPhase === 1 ? "animate-pulse" : "animate-bounce"}`}>
                      <svg className="w-6 h-6 text-indigo-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round">
                  ...
                  <>
                    ...
                    <div className="text-sm font-semibold text-gray-900 mb-2">Profile built</div>
                    <div className="flex flex-wrap gap-1.5 justify-center">
                      {MOCK_SKILLS.map((skill, i) => (
                        <span
                          key={skill}
                          className="text-[10px] px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-700 border border-indigo-200 animate-fade-up"
              ...
              <div className={`w-full max-w-sm grid grid-cols-2 gap-2 transition-all duration-500 ${uploadPhase === 2 ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
                {[
                  ...
                  <div key={pref.label} className="bg-gray-50 border border-gray-100 rounded-lg px-3 py-2">
                    ...
                    <div className="text-xs text-gray-900 font-medium">{pref.value}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="h-full flex flex-col">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-indigo-100 flex items-center justify-center">
                    <svg className={`w-4 h-4 text-indigo-600 ${scanCount < 2847 ? "animate-spin" : ""}`} style={{ animationDuration: "2s" }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
                      <circle cx="12" cy="12" r="10" /><circle cx="12" cy="12" r="6" /><circle cx="12" cy="12" r="2" />
                    </svg>
                  </div>
                  <div>
                    <div className="text-sm font-bold text-gray-900">Nightly scan running</div>
                    <div className="text-[10px] text-gray-400">2:00 AM EST — while you sleep</div>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-lg font-extrabold text-indigo-600 tabular-nums">{scanCount.toLocaleString()}</div>
                  <div className="text-[10px] text-gray-400">jobs scanned</div>
                </div>
              </div>

              <div className="h-1 rounded-full bg-gray-100 mb-6 overflow-hidden">
                <div
              ...
              <div className="flex-1 space-y-2">
                {MOCK_MATCHES.map((match, i) => (
                  <div
                    key={match.company}
                    className={`flex items-center gap-4 bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 transition-all duration-500 ${
                  ...
                  >
                    <div className={`w-9 h-9 rounded-lg flex items-center justify-center text-xs font-extrabold flex-shrink-0 ${
                      match.grade === "A" ? "bg-emerald-100 text-emerald-700" : "bg-blue-100 text-blue-700"
                    }`}>
                      {match.grade}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-semibold text-gray-900 truncate">{match.title}</div>
                      <div className="text-xs text-gray-500">{match.company}</div>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <div className="text-sm font-bold text-indigo-600">{match.score}%</div>
                ...
                <div className={`text-center pt-2 transition-all duration-500 ${matchesRevealed >= 3 ? "opacity-100" : "opacity-0"}`}>
                  <span className="text-xs text-gray-500">
                    <span className="text-indigo-600 font-bold">3 matches</span> found from 2,847 listings
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className={`absolute inset-0 p-6 sm:p-8 transition-all duration-500 ${
            ...
            <div className="h-full flex flex-col">
              <div className="border border-gray-200 rounded-xl bg-white mb-3">
                <div className="px-4 py-2.5 border-b border-gray-100">
                  <div className="flex items-center gap-2 mb-1.5">
                    ...
                    <span className="text-xs text-gray-900 font-medium">{MOCK_OUTREACH.to}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] text-gray-400 w-8">Re:</span>
                    <span className="text-xs text-gray-600">{MOCK_OUTREACH.subject}</span>
                  </div>
                </div>
                ...
                <div className="px-4 py-3 min-h-[200px]">
                  {MOCK_OUTREACH.lines.map((line, i) => (
                    <div
                      ...
                      } ${line === "" ? "h-3" : "text-gray-700"}`}
                    >
                      {line}
                      {i === outreachLines - 1 && outreachLines < MOCK_OUTREACH.lines.length && (
                        <span className="inline-block w-[2px] h-3 bg-indigo-500 ml-0.5 animate-pulse" />
                      )}
                    </div>
                  ))}
                </div>
              ...
              <div className={`flex items-center gap-3 transition-all duration-500 ${
                ...
                <div>
                  <div className="text-xs font-semibold text-emerald-700">Outreach sent</div>
                  <div className="text-[10px] text-gray-400">3 personalized messages delivered anonymously</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className={`absolute inset-0 p-6 sm:p-8 transition-all duration-500 ${
            ...
            <div className="h-full flex flex-col items-center justify-center gap-6">
              <div className={`w-full max-w-[280px] transition-all duration-700 ${
                ...
                <div className={`bg-white border border-gray-200 rounded-2xl p-4 transition-all duration-500 ${
                  notifyPhase >= 1 ? "shadow-lg shadow-indigo-100" : ""
                }`}>
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-gray-900 font-medium mb-0.5">
              ...
              <div className={`flex gap-3 transition-all duration-500 ${
                ...
                <button className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-500 text-white text-xs font-bold shadow-md shadow-indigo-200">
                  Accept intro
                </button>
                <button className="px-5 py-2.5 rounded-xl bg-gray-50 border border-gray-200 text-gray-600 text-xs font-medium">
                  Pass
                </button>
              </div>
